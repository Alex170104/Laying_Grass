...
displayWin::displayWin(vector<Player> orderWinners) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < orderWinners.size(); i++) {
            DrawRectangle(95, 218 + 50 * i, 600, 50, Fade(orderWinners[i].getColor(), 0.25f));
            ...
            DrawText(to_string(orderWinners[i].getBiggestSquare()).c_str(), 450, 230 + 50 * i, 30, orderWinners[i].getColor());
